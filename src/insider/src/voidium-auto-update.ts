...
export class VoidiumAutoUpdateService {
    ...
    private constructor() {
        ...
        this.isInsiders = true; // Always insiders for this build
    }
        ...
        try {
            ...
            if (!latestRelease) {
                if (showNoUpdateMessage) {
                    vscode.window.showInformationMessage('Voidium Code: No pre-release updates available.');
                }
                return;
            }
            ...
            if (comparison <= 0) {
                if (showNoUpdateMessage) {
                    vscode.window.showInformationMessage(`Voidium Code: You are on the latest pre-release (${this.currentVersion}).`);
                }
                return;
            }
            ...
            const req = https.request(options, (res) => {
                ...
                res.on('end', () => {
                    try {
                        ...
                        // Insiders: ONLY get prereleases (pre-releases)
                        let filteredReleases = releases.filter(r => !r.draft && r.prerelease);
    ...
    private async promptForUpdate(release: GitHubRelease): Promise<void> {
        const message = `Voidium Code Insiders Pre-release ${release.tag_name} is available. Current: ${this.currentVersion}`;
            ...
            https.get(url, { headers: { 'User-Agent': 'Voidium-Code' } }, (response) => {
                ...
                response.on('data', (chunk) => {
                    ...
                    if (totalSize > 0) {
                        ...
                        progress.report({ increment: percent / 100 });
                    }
                });
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    statusBarItem.text = "$(flame) Voidium Insiders";
    statusBarItem.tooltip = "Check for Voidium Code Insiders pre-release updates";
